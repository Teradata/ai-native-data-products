...
> Install the `access` skill first (generated by the Skill Conversion Prompt). It carries the product-first discovery order and the pre-use trust gate. For an always-on consumer, the skill's `SKILL.md` can also be embedded directly in the agent's system prompt.
